...
const CodeViewer: React.FC<CodeViewerProps> = ({ file, vulnerabilities }) => {
  const fileVulns = file
    ? vulnerabilities.filter((v) => v.file === file.path)
    : [];
          ...
          <div className="bg-white border-r border-gray-200">
            <pre className="text-sm">
              {lines.map((line, index) => {
                ...
                const lineVulns = fileVulns.filter(
                  (v) => v.line === lineNumber
                );
                ...
                return (
                  <div key={index} className="flex">
                    <div
                      className={`w-12 px-2 py-1 text-right text-xs text-gray-500 bg-gray-50 border-r border-gray-200 select-none ${
                        hasVulnerability ? "bg-red-50" : ""
                      }`}
                    >
                      {lineNumber}
                    </div>
                    <div
                      className={`flex-1 px-4 py-1 font-mono text-sm ${
                        hasVulnerability ? "bg-red-50" : ""
                      }`}
                    >
                        ...
                        <div className="mt-2 space-y-2">
                          {lineVulns.map((vuln) => {
                            ...
                            return (
                              <div
                                key={vuln.id}
                                className={`p-3 rounded-lg border ${getVulnerabilityColor(
                                  vuln.type
                                )}`}
                              >
                                <div className="flex items-start space-x-2">
                                  <VulnIcon.component
                                    className={VulnIcon.class}
                                  />
